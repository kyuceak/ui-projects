...
function ProjectExperienceForm({ projects, setProjects,resetTrigger }) {
  ...
  const updateProjectExperience = (updatedProject) => {
    setProjects(
  ...
  return (
    <div className="my-container">
        ...
        {projects.map((project) =>
          editingId === project.id ? (
            <ProjectForm
              key={project.id}
              education={project}
              updateEducationalExperience={updateProjectExperience}
              removeEducationExperience={removeProjectExperience}
      ...
      {editingId !== null ? null : (
        <button className="pls-btn" onClick={addProjectExperience}>
          <svg
            viewBox="0 -0.5 21 21"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
          ...
          <span className="pls-text">Project</span>
        </button>
      )}
    </div>
  );
